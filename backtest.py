...
class NBABacktester:
    ...
    def evaluate_prediction(self, row, prediction):
        ...
        actual_winner = row["Home"] if row["HomePoints"] > row["AwayPoints"] else row["Away"]
    ...
    def run_backtest(self):
        ...
        if not self.retrain_per_game:
            ...
            with Pool(processes=cpu_count()) as pool:
                ...
                chunks = [group for _, group in valid_games.groupby(np.arange(len(valid_games)) // chunk_size)]
                results = []
                for chunk in tqdm(chunks, desc="Processing games"):
                    chunk_results = [self.evaluate_prediction(row, self.model.predict_future_games(pd.DataFrame([row]))) 
...
def run_comparative_backtest(batch_interval='week'):
    ...
    batch_backtester = NBABacktester(retrain_per_game=True, batch_interval=batch_interval)
    ...
    combined_summary = pd.DataFrame({
        'Metric': ['TotalGames', 'CorrectPredictions', 'Accuracy', 'AverageSpreadError'],
        'SingleModel': [
...
def main():
    ...
    parser.add_argument('--batch-interval', choices=['month', 'week', 'day'],
                       default='month', help='Interval for model retraining')
