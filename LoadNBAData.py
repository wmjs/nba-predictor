...
class NBADataLoader:
    ...
    def get_nba_team_abbreviations(self):
        ...
        self.nba_abbreviations = (
            ...
            .rename(
                columns={"Franchise": "Team", "Abbreviation/ Acronym": "Abbreviation"}
            )
            .set_index("Team")
        )
        self.nba_abbreviations["Abbreviation"] = (
            self.nba_abbreviations["Abbreviation"].str.split().str[0]
        )
    ...
    def get_raw_schedule_and_results(self):
        ...
        for month in season_months:
            ...
            if schedule_and_results_raw_month["HomePoints"].isnull().all():
                ...
                continue
            schedule_and_results_raw_month = schedule_and_results_raw_month[
                schedule_and_results_raw_month["HomePoints"].notna()
            ]
            schedule_and_results_raw = pd.concat(
                [schedule_and_results_raw, schedule_and_results_raw_month]
            )
        # Clean up columns
        columns_to_drop = [
            "Unnamed: 6",
        ...
        print(
            "DATE1",
            pd.to_datetime(schedule_and_results_raw["Date"]).max(),
            pd.to_datetime(schedule_and_results_raw["Date"]).min(),
        )
    ...
    def get_schedule_no_results(self):
        ...
        for month in season_months:
            ...
            schedule_no_results_raw_month = schedule_no_results_raw_month[
                schedule_no_results_raw_month["HomePoints"].isnull()
            ]
            schedule_no_results_raw = pd.concat(
                [schedule_no_results_raw, schedule_no_results_raw_month]
            )
        # Clean up columns
        columns_to_drop = [
            "Unnamed: 6",
    ...
    def set_up_schedule_and_results(self):
        ...
        if self.schedule_and_results_raw is None or self.nba_abbreviations is None:
            raise ValueError(
                "Must call get_raw_schedule_and_results() and get_nba_team_abbreviations() first"
            )

        def get_first_game_dates(df):
            """Get the first game date for each team, both home and away."""
            first_home = (
                df.groupby("Home")["Date"].min().reset_index().set_index("Home")
            )
            first_away = (
                df.groupby("Away")["Date"].min().reset_index().set_index("Away")
            )

            first_games = pd.merge(
                first_home, first_away, left_index=True, right_index=True
            )
            first_games.columns = ["FirstGameHome", "FirstGameAway"]
            first_games["FirstGame"] = first_games[
                ["FirstGameHome", "FirstGameAway"]
            ].min(axis=1)
        ...
        def is_first_game(row, first_games):
            ...
            return (
                row["Date"] == first_games.loc[row["Home"]].values[0]
                or row["Date"] == first_games.loc[row["Away"]].values[0]
            )
        ...
        self.schedule_and_results_raw["Date"] = pd.to_datetime(
            self.schedule_and_results_raw["Date"]
        ).dt.strftime("%Y%m%d")
        self.schedule_and_results_raw["HomeAbbreviation"] = (
            self.schedule_and_results_raw["Home"].map(
                self.nba_abbreviations["Abbreviation"]
            )
        )
        self.schedule_and_results_raw["AwayAbbreviation"] = (
            self.schedule_and_results_raw["Away"].map(
                self.nba_abbreviations["Abbreviation"]
            )
        )
        ...
        # Generate game URLs
        base_game_url = (
            "https://www.basketball-reference.com/boxscores/{date}0{home_team}.html"
        )
        self.schedule_and_results_raw["GameUrl"] = self.schedule_and_results_raw.apply(
            lambda x: base_game_url.format(
                date=x["Date"], home_team=x["HomeAbbreviation"]
            ),
            axis=1,
        )
        ...
        self.schedule_and_results_raw["FirstGame"] = (
            self.schedule_and_results_raw.apply(
                lambda row: is_first_game(row, first_games), axis=1
            )
        )
        self.schedule_and_results_raw = self.schedule_and_results_raw[
            ~self.schedule_and_results_raw["FirstGame"]
        ].reset_index(drop=True)

        # Add consistent team ordering columns
        self.schedule_and_results_raw["A_Team"] = self.schedule_and_results_raw[
            ["Home", "Away"]
        ].min(axis=1)
        self.schedule_and_results_raw["B_Team"] = self.schedule_and_results_raw[
            ["Home", "Away"]
        ].max(axis=1)
        ...
        self.schedule_and_results_raw["Winner"] = self.schedule_and_results_raw.apply(
            get_game_winner, axis=1
        )
        self.schedule_and_results_raw["PointDifferential"] = (
            self.schedule_and_results_raw.apply(get_point_differential, axis=1)
        )

        self.schedule_and_results = self.schedule_and_results_raw
        self.schedule_and_results["Date"] = pd.to_datetime(
            self.schedule_and_results["Date"]
        )
    ...
    def scrape_game_page(self, url):
        ...
        stat_dict = {
            "away_basic_stats": process_team_stats(
                table_list[away_basic_stats_index], "away_basic_"
            ),
            "home_basic_stats": process_team_stats(
                table_list[home_basic_stats_index], "home_basic_"
            ),
            "away_advanced_stats": process_team_stats(
                table_list[away_advanced_stats_index], "away_advanced_"
            ),
            "home_advanced_stats": process_team_stats(
                table_list[home_advanced_stats_index], "home_advanced_"
            ),
        }
        ...
        if reload or self.load_new:
            if self.load_new and not reload:
                ...
                try:
                    ...
                    self.latest_combined_stats_date = (
                        self.get_latest_combined_stats_date()
                    )

                    # Get new games only
                    print(
                        f"Last loaded: {self.latest_combined_stats_date}, loading as of today: {self.schedule_and_results['Date'].max()}"
                    )
                    new_schedule = self.schedule_and_results[
                        pd.to_datetime(
                            self.schedule_and_results["Date"], format="%Y%m%d"
                        )
                        > self.latest_combined_stats_date
                    ]
                    ...
                    self.schedule_and_results = pd.concat(
                        [new_schedule, self.schedule_and_results], ignore_index=True
                    )
                    self.schedule_and_results.drop_duplicates(
                        keep="first", inplace=True
                    )

                    # Combine old and new data
                    self.combined_stats = pd.concat(
                        [self.combined_stats, new_stats], ignore_index=True
            ...
            try:
                self.combined_stats = pd.read_csv(
                    os.path.join(
                        self.data_folder, "combined_schedule_and_game_stats.csv"
                    ),
                    index_col=0,
                )
                self.combined_stats["Date"] = pd.to_datetime(
                    self.combined_stats["Date"], format="%Y%m%d"
                )
            except FileNotFoundError:
    ...
    def split_home_away_games(self):
        ...
        home_columns = [
            col for col in home_games.columns if col.startswith("home_")
        ] + ["Home", "Date"]
        ...
        away_columns = [
            col for col in away_games.columns if col.startswith("away_")
        ] + ["Away", "Date"]
    ...
    def calculate_cumulative_team_stats(self):
        ...
        cumulative_home_stats = cumulative_home_stats.groupby(
            "Team", group_keys=False
        ).apply(lambda x: x.sort_index(level="Date"))
        cumulative_away_stats = cumulative_away_stats.groupby(
            "Team", group_keys=False
        ).apply(lambda x: x.sort_index(level="Date"))

        # Calculate cumulative averages for each numeric column within each team group
        numeric_columns_home = cumulative_home_stats.select_dtypes(
            include=["int64", "float64"]
        ).columns
        numeric_columns_away = cumulative_away_stats.select_dtypes(
            include=["int64", "float64"]
        ).columns

        cumulative_home_stats[numeric_columns_home] = (
            cumulative_home_stats.groupby("Team")[numeric_columns_home]
            .expanding()
            .mean()
            .reset_index(0, drop=True)
        )
        cumulative_away_stats[numeric_columns_away] = (
            cumulative_away_stats.groupby("Team")[numeric_columns_away]
            .expanding()
            .mean()
            .reset_index(0, drop=True)
        )
    ...
    def enhance_schedule_with_team_stats(self):
        ...
        def get_latest_stats(team, date, stats_df):
            ...
            team_stats_before_date = team_stats[
                team_stats.index.get_level_values("Date") <= date
            ]
            return (
                team_stats_before_date.iloc[-1]
                if not team_stats_before_date.empty
                else None
            )
        ...
        for column in self.all_team_stats.columns:
            enhanced_schedule[f"Home_{column}"] = enhanced_schedule.apply(
                lambda row: (
                    get_latest_stats(row["Home"], row["Date"], self.all_team_stats)[
                        column
                    ]
                    if get_latest_stats(row["Home"], row["Date"], self.all_team_stats)
                    is not None
                    else None
                ),
                axis=1,
        ...
        for column in self.all_team_stats.columns:
            enhanced_schedule[f"Away_{column}"] = enhanced_schedule.apply(
                lambda row: (
                    get_latest_stats(row["Away"], row["Date"], self.all_team_stats)[
                        column
                    ]
                    if get_latest_stats(row["Away"], row["Date"], self.all_team_stats)
                    is not None
                    else None
                ),
                axis=1,
    ...
    def load_all_from_files(self):
        ...
        try:
            self.nba_abbreviations = pd.read_csv(
                os.path.join(self.data_folder, "nba_team_abbreviations.csv"),
                index_col=0,
            )
            self.schedule_and_results = pd.read_csv(
                os.path.join(self.data_folder, "schedule_and_results.csv"), index_col=0
            )
            self.combined_stats = pd.read_csv(
                os.path.join(self.data_folder, "combined_schedule_and_game_stats.csv"),
                index_col=0,
            )
            self.home_games_df = pd.read_csv(
                os.path.join(self.data_folder, "home_games_stats.csv"), index_col=[0, 1]
            )
            self.away_games_df = pd.read_csv(
                os.path.join(self.data_folder, "away_games_stats.csv"), index_col=[0, 1]
            )
            self.all_team_stats = pd.read_csv(
                os.path.join(self.data_folder, "all_team_stats.csv"), index_col=[0, 1]
            )
            self.enhanced_schedule = pd.read_csv(
                os.path.join(self.data_folder, "enhanced_schedule.csv"), index_col=0
            )
            self.schedule_no_results = pd.read_csv(
                os.path.join(self.data_folder, "schedule_no_results.csv"), index_col=0
            )
            ...
            for table_name, df in date_tables.items():
                if isinstance(df.index, pd.MultiIndex):
                    ...
                    if df.index.get_level_values("Date").dtype == "int64":
                        date_level = pd.to_datetime(
                            df.index.get_level_values("Date"), format="%Y%m%d"
                        )
                    else:
                    ...
                    new_index = pd.MultiIndex.from_arrays(
                        [date_level, team_level], names=["Date", "Team"]
                    )
                    df.index = new_index
