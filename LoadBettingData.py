...
def get_odds(odds_type, abv):
    ...
    def map_to_dict(lst):
        return {"Away": lst[0], "Home": lst[1], "Time": lst[2]}
    ...
    df_grouped["Away-Matchup"] = df_grouped["Away-Matchup"].apply(
        lambda x: "".join([c for c in x if c.isalpha()])
    )
    df_grouped["Home-Matchup"] = df_grouped["Home-Matchup"].apply(
        lambda x: "".join([c for c in x if c.isalpha()])
    )
    ...
    df_grouped.rename(
        columns={
            "Away-Matchup": "Away",
            "Home-Matchup": "Home",
            "Away-consensus": f"Away odds",
            "Home-consensus": f"Home odds",
        },
        inplace=True,
    )
    df_grouped = df_grouped.drop(
        columns=["Time-Matchup", "Time-consensus"]
    ...
    nice_odds = {
        ...
        "picks-against-the-spread": "Spread",
        "over-under": "Over/Under",
    }

    df_grouped["Away Line"] = df_grouped.apply(
        lambda row: clean_odds(row, hoa="Away", odds_type=odds_type), axis=1
    )
    df_grouped["Home Line"] = df_grouped.apply(
        lambda row: clean_odds(row, hoa="Home", odds_type=odds_type), axis=1
    )
    ...
    df_grouped[
        [
            f"Away {nice_odds[odds_type]}",
            f"Away {nice_odds[odds_type]} Odds",
            f"Away {nice_odds[odds_type]} Implied Probability",
        ]
    ] = pd.DataFrame(df_grouped["Away Line"].tolist(), index=df_grouped.index)
    df_grouped[
        [
            f"Home {nice_odds[odds_type]}",
            f"Home {nice_odds[odds_type]} Odds",
            f"Home {nice_odds[odds_type]} Implied Probability",
        ]
    ] = pd.DataFrame(df_grouped["Home Line"].tolist(), index=df_grouped.index)
    ...
    out = (
        df_grouped.merge(abv, left_on="Away", right_on="Team Name", how="left")
        .drop(columns=["Abbreviation", "Team Name", "Away"])
        .rename(columns={"Team": "Away"})
        .merge(abv, left_on="Home", right_on="Team Name", how="left")
        .drop(columns=["Abbreviation", "Team Name", "Home"])
