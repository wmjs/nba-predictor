...
class NBAPredictionModel:
    ...
    def train(self, df, num_epochs=200, batch_size=16):
        ...
        train_dataset = NBADataset(
            torch.FloatTensor(X_scaled), torch.FloatTensor(y_scaled)
        )
        ...
        for epoch in range(num_epochs):
            ...
            if avg_val_loss < best_val_loss:
                ...
                # Save best model
                torch.save(
                    self.model.state_dict(), self.data_folder + "/best_model.pth"
                )
            else:
                patience_counter += 1
            ...
            if (epoch + 1) % 10 == 0:
                ...
                if self.verbose:
                    print(
                        f"Epoch [{epoch+1}/{num_epochs}], Train Loss: {avg_loss:.4f}, Val Loss: {avg_val_loss:.4f}"
                    )

                # Load best model
        self.model.load_state_dict(
            torch.load(self.data_folder + "/best_model.pth", weights_only=True)
        )
    ...
    def _format_predictions(self, future_games, predictions):
        ...
        future_games["PredictedSpread"] = future_games.apply(
            self._predicted_spread, axis=1
        )
        future_games["PredictedWinner"] = future_games.apply(
            self._predicted_winner, axis=1
        )

        prediction_display = future_games[
            [
                "Date",
                "Away",
                "PredictedAwayPoints",
        ...
        prediction_display["PredictedAwayPoints"] = prediction_display[
            "PredictedAwayPoints"
        ].round(1)
        prediction_display["PredictedHomePoints"] = prediction_display[
            "PredictedHomePoints"
        ].round(1)
        prediction_display["PredictedSpread"] = prediction_display[
            "PredictedSpread"
        ].round(1)
        ...
        prediction_display["Confidence"] = prediction_display["PredictedSpread"].apply(
            lambda x: round(
                ...
                / (
                    1
                    + np.exp(
                        -6 * (x - min_spread) / (max_spread - min_spread + epsilon) + 3
                    )
                ),
                2,
            )
    ...
    def prepare_latest_games(self):
        ...
        latest_home_games = (
            self.enhanced_schedule.sort_values("Date")
            .groupby("Home")
            .last()
            .reset_index()
        )
        ...
        latest_games = pd.concat(
            [
                latest_home_games[["Home", "Date"]]
                .rename(columns={"Home": "Team"})
                .assign(Location="Home"),
                latest_away_games[["Away", "Date"]]
                .rename(columns={"Away": "Team"})
                .assign(Location="Away"),
            ]
        )
        latest_games = (
            latest_games.sort_values("Date").groupby("Team").last().reset_index()
        )
        ...
        for tup in latest_games.itertuples():
            ...
            cols = [
                col
                for col in self.enhanced_schedule.columns
                if str(col).startswith(prefix)
            ]
            latest_game_stats_team = self.enhanced_schedule[
                (self.enhanced_schedule[tup.Location] == tup.Team)
                & (self.enhanced_schedule["Date"] == tup.Date)
            ][cols]
            latest_game_stats_team["Team"] = tup.Team
            latest_game_stats_team.rename(
                columns={
                    col: col.replace(prefix, "")
                    for col in latest_game_stats_team.columns
                },
                inplace=True,
            )
    ...
    def prepare_future_games(self):
        ...
        future_games = (
            self.schedule_no_results.merge(
    ...
    def _predicted_winner(row):
        return (
            row["Home"]
            if row["PredictedHomePoints"] > row["PredictedAwayPoints"]
            else row["Away"]
        )
